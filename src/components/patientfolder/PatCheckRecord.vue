<template>
  ...
  <div>
    ...
    <mt-cell>
      ...
      <mt-button size="small" icon="more"></mt-button>
    </mt-cell>
    <div @click="JumpPage()">
      <mt-navbar v-model="selectedRecord">
        ...
        <mt-tab-item id="2">评估记录</mt-tab-item>
      </mt-navbar>
    </div>
    <div>
      评估记录
      <mt-cell title="评估记录跳转" label="跳转到具体评估记录页" is-link to="/patient/evaluation"></mt-cell>
    </div>
    <div @click="JumpPage()">
  ...
  methods: {
    JumpPage () {
      if (this.selectedAppointment == '2') {
        //跳转到预约问诊
        this.$router.push({ name: "/patient/appt/consulation" });
      }
      else if (this.selectedRecord == '1') {
        //跳转到记录页
